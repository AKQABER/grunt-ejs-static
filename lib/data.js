...
module.exports = exports = {
  ...
  get_data: function(key, files) {
    ...
    function process_string_data(this_file_data, additional_data) {
      ...
      if (this_file_data[the_filename]) {
        var extradata = grunt.file.readJSON(additional_data);
        for (var name in extradata) {
          if (extradata.hasOwnProperty(name)) {
            this_file_data[the_filename][name] = extradata[name];
          }
        }
      } else {
