      ...
      // return the data
        ...
        for (var page in files_data) {
          if (files_data.hasOwnProperty(page)) {
            ...
            files_data[page].lang = options.data.i18n.split('/')[options.data.i18n.split('/').length -1].replace('.json', '');
            if (options.data.path.global) {
              files_data[page].global = options.data.path.global;
            }
          }
        }
      }
